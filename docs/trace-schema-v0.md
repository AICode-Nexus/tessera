...
Automatic isolated worktree lifecycle metadata is recorded through `workspace_worktree_lifecycle_recorded`. The payload records generated worktree id, workflow/task refs, trace id, source commit, optional source branch label, redacted worktree root label, redacted base key, lifecycle status (`planned`, `created`, `creation_failed`, `retained`, `cleanup_started`, `cleanup_completed`, `cleanup_failed`), reason, optional mutation request/patch refs and bounded evidence refs. `tessera worktree cleanup` may append `cleanup_started` and terminal cleanup lifecycle records only after validating retained lifecycle evidence for the requested worktree id plus an explicit local path; dry-run appends nothing. The CLI may return the local generated worktree path directly to the operator, but trace payloads must not include absolute source checkout paths, absolute worktree paths, Git command strings, stdout/stderr bodies, file contents, checkpoint restore commands, stage/commit/push commands, provider-private responses, hidden reasoning or secrets.
