...
Apply-patch preflight and dry-run summaries are core metadata, not executor proof. If a future trace event records them, it may include affected relative paths, operation summaries and blocker labels, but must not include patch body text, workspace file contents, external command handles, worktree filesystem handles or applied-patch claims.
