...
v0.7 coding-agent workflow foundation adds trace events for workflow scope, mutation request proposals, patch proposals, diff artifact refs, explicit artifact body handles, checkpoint lifecycle records, checkpoint/reviewer requirements, test evidence refs, metadata-only test evidence summaries, review bundles and restore plans. Except for the narrow explicit and trace-driven apply-patch CLI slices, opt-in detached auto-worktree lifecycle creation for trace-driven apply-patch, and conservative trace-backed worktree cleanup, these events are metadata-only until future policy/sandbox/checkpoint/reviewer-gated executors apply patches, run tests, restore checkpoints or mutate Git.

Apply-patch preflight and dry-run summaries are core metadata, not executor proof. `apply_patch_preflight_recorded` may include affected relative paths, operation summaries, blocker labels, evidence refs and an executor-blocked reason, but must not include patch body text, workspace file contents, external command handles, worktree filesystem handles or applied-patch claims.

Apply-patch execution metadata is distinct from patch application workflow summaries. `apply_patch_execution_recorded` records executor intent/result metadata bound to an executor-ready preflight id, checkpoint/reviewer/policy/sandbox refs, isolated root label, executor label, affected paths, conflict paths, blockers and artifact refs. It must not include patch body text, file contents, stdout/stderr bodies, provider-private responses, hidden reasoning, secrets, primary-root absolute paths or raw filesystem handles.

The explicit and trace-driven `tessera apply-patch` CLI tool records preflight metadata for every dry-run or apply attempt that reaches the core gate, and execution metadata only when the core executor is invoked. Trace-driven `--auto-worktree` non-dry apply additionally records path-redacted worktree lifecycle metadata around the same preflight/execution records; auto-worktree dry-run does not create a worktree or append lifecycle records. Trace records contain IDs, labels, affected paths, blockers and evidence references; they do not contain patch bodies, old file contents, new file contents or absolute isolated-root/worktree/source filesystem paths.
...
Automatic isolated worktree lifecycle metadata is recorded through `workspace_worktree_lifecycle_recorded`. The payload records generated worktree id, workflow/task refs, trace id, source commit, optional source branch label, redacted worktree root label, redacted base key, lifecycle status (`planned`, `created`, `creation_failed`, `retained`, `cleanup_started`, `cleanup_completed`, `cleanup_failed`), reason, optional mutation request/patch refs and bounded evidence refs. Read-only consumers may project this data through `RuntimeReader::list_worktree_lifecycles`, `tessera-client` worktree lifecycle status, `tessera worktree list --trace <trace_id> [--json]`, TUI status summaries and GUI metadata panels. `trace_cleanup_candidate` is only a conservative trace-evidence label for generated retained worktrees; cleanup still requires explicit local path validation. `tessera worktree cleanup` may append `cleanup_started` and terminal cleanup lifecycle records only after validating retained lifecycle evidence for the requested worktree id plus an explicit local path; dry-run appends nothing. The CLI may return the local generated worktree path directly to the operator, but trace payloads must not include absolute source checkout paths, absolute worktree paths, Git command strings, stdout/stderr bodies, file contents, checkpoint restore commands, stage/commit/push commands, provider-private responses, hidden reasoning or secrets. GUI metadata panels must further omit touched paths, workflow objective text, reason text, summaries, diagnostics, artifact bodies and mutation controls.

## 4. Event Kind

当前必须支持（v0.1 基线 + v0.2-v0.7 foundation/runtime signals）：
...
`runtime_instance_started` payload 必须包含 `instance`，其中 runtime id、started_at 和 capabilities 是安全 metadata，process id、hostname 和 working_directory 均为可选。`task_owner_attached` payload 必须包含 `lease`，`task_owner_heartbeat` payload 必须包含 `heartbeat`，`task_owner_detached` / `task_owner_lost` payload 必须包含 `lease_id`、`task_id` 和可选 `reason`，`task_reattach_recorded` payload 必须包含 `record`。这些 ownership events 只允许保存 runtime/client/lease/status/seq/reason/checkpoint metadata；不得保存 provider socket、headers、API key、cookie、env、命令行 secret、tool output、workspace diff 或文件内容。当前实现可通过 `RuntimeReader::list_task_owners`、`TaskOwnershipRecorder`、client projection 和 `tessera tasks --owners --trace <trace_id>` 消费这些事件，并会在 no-tool chat / agent traces 中自动记录 `task_owner_attached` / `task_owner_detached`。它仍不提供 daemon、app-server listener、provider socket freezing 或跨进程后台执行保证。

Runtime API / app-server alignment DTOs are not trace events by themselves. They define localhost-safe bind metadata, auth policy metadata, queue bounds, command envelopes, and event stream request shape for future transports. If a future app-server executes a mutation command, that command must first map to a core runtime command or `ClientIntent` and then produce the same trace events as CLI/TUI/GUI paths; no app-server side table may become event truth.
...
`coding_workflow_started` payload must contain `workflow_id`, `task_id` and `objective`. `workspace_mutation_scope_recorded` payload must contain `scope`, including workflow id, task id, root label, allowed/denied relative paths, mutation mode, worktree requirement and optional reason. `mutation_request_proposal_recorded` payload must contain `proposal`, including request id, workflow id, task id, neutral operation kind label, status, summary, requested paths, optional checkpoint id, optional reviewer gate id, optional policy decision id, optional sandbox profile label, worktree requirement and bounded evidence refs. Operation kind labels describe proposed outcomes such as `patch_application`, `test_run`, `checkpoint_restore` or `version_control_commit`; they must not be GUI/runtime command names. `artifact_body_recorded` payload must contain `record`, including artifact id, kind, optional task id, media type, byte length, storage URI, redaction status and summary; it must not contain artifact body bytes. `apply_patch_preflight_recorded` payload must contain `record`, including preflight id, workflow id, task id, mutation request id, patch proposal id, readiness status, blocker labels, affected relative paths, dry-run operation summaries, bounded evidence refs and an `executor_blocked` reason. `apply_patch_execution_recorded` payload must contain `record`, including execution id, preflight id, workflow id, task id, mutation request id, patch proposal id, optional checkpoint/reviewer/policy refs, optional sandbox profile label, isolated root label, executor label, status, blocker labels, affected path summaries, conflict path summaries, artifact refs and bounded evidence refs. `workspace_worktree_lifecycle_recorded` payload must contain `record`, including worktree id, workflow id, task id, trace id, source commit, optional source branch label, redacted root label, redacted base key, lifecycle status, reason, optional mutation request/patch refs, bounded evidence refs and safe metadata. `patch_proposal_recorded` payload must contain `proposal`, including patch id, workflow id, task id, summary, touched paths, diff artifact refs, optional checkpoint id and optional reviewer gate id. `patch_application_recorded` payload must contain `record`, including patch id, workflow id, task id, optional checkpoint id, outcome, conflict/applied path summaries and artifact refs; it remains a workflow summary/projection and is not the sole proof of runtime execution. `test_plan_recorded` payload must contain `plan`; `test_run_recorded` payload must contain `record` with artifact refs for stdout/stderr rather than inline command output. `test_evidence_summary_recorded` payload must contain `record` with summary id, workflow/task ids, test plan/run ids, aggregate status, run counts, artifact refs, diagnostics, redaction status, summary text and `execution_blocked: true`; it must not contain stdout/stderr bodies or command execution handles. `review_bundle_recorded` payload must contain `bundle`; `restore_plan_recorded` payload must contain `plan` with `execution_blocked` for metadata-only restore plans.

These coding workflow events must not contain patch bodies, file contents, stdout/stderr bodies, absolute source/worktree filesystem paths, shell command execution handles, provider-private handles, API keys, cookies, authorization headers, hidden reasoning, checkpoint restore commands or Git mutation commands. They are replayable evidence and gate metadata; except for the narrow isolated apply-patch executor slice recorded by `apply_patch_execution_recorded` and the opt-in detached worktree lifecycle around trace-driven apply-patch, they do not apply patches, write files, run commands, restore checkpoints, stage commits, push branches or open PRs. Current TUI/GUI surfaces are read-only projections over safe aggregates; they do not add mutation intents, provider calls, app-server mutation listener behavior or storage ownership.

Read-only workflow inspection is a client projection over the trace events above, not a new trace event. `ClientWorkflowInspection`, `tessera workflow inspect --trace <trace_id> [--json]`, TUI `workflow_inspection_summary` and the GUI Review Inspection panel may derive generated workflow/task refs, counts, booleans and status labels from replayed coding workflow, reviewer gate and approval records, including empty-trace output. They must not load artifact bodies, render patch text, expose raw ids, paths, objectives, summaries, diagnostics, comments or reason text, execute tests, restore checkpoints, apply diffs, stage/commit/push Git, execute shell/provider tools, start an app-server mutation listener or add GUI/TUI mutation controls.

Apply-patch dry-run metadata may be derived from an artifact body already loaded through explicit artifact APIs, but the trace must keep only summaries such as affected paths, supported/unsupported operation labels and blocker reasons. Dry-run metadata must not be interpreted as a patch application result.
