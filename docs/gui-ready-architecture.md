...
- v0.1：补 TUI profile switch 时，先按 client intent 设计，不把 profile 选择写成 Ratatui 私有逻辑。
- v0.1：补 live event bridge，让 CLI/TUI 先消费同一套 event stream，并保证 GUI 后续复用同一契约。
