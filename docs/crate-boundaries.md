...
- `SubagentRuntimeCoordinator` ownership: scheduler decisions, cap validation, transcript artifact lifecycle metadata, approval-forwarding policy records, inactive-child policy records and cancellation/reattach metadata before any child execution. Current implementation is non-executing and only returns provider-neutral decision/lifecycle/approval/inactive-policy metadata events; it can reserve, publish, seal or abandon transcript artifact handles and validate approval-forwarding / inactive-policy records without storing transcript bodies or forwarding approvals automatically.
