...
- `ApplyPatchExecutor` ownership: pure in-memory v0.7 single-file patch parsing/application model for future executor slices. Current implementation accepts patch body text plus optional current file contents and returns provider-neutral execution status, blockers, affected paths, conflict paths and new contents. It supports one UTF-8 text create/modify operation only and rejects delete, rename, binary and multi-file patches. It must not call `std::fs`, `std::process::Command`, Git, providers, storage, worktree creation, checkpoint restore or UI.
