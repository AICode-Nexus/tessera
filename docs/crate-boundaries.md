...
- `ApplyPatchGate` ownership: non-mutating v0.7 apply-patch preflight validation over existing mutation request, patch proposal, scope, enforcement, checkpoint, reviewer and policy metadata. Current implementation only returns readiness/blocker metadata and keeps `executor_blocked` true; it does not parse or apply patch bodies, read workspace files, create worktrees, run commands, restore checkpoints, mutate Git, call providers, write storage directly or drive UI.
