...
- v0.7 已具备 metadata-only coding workflow foundation、窄的 explicit `tessera apply-patch` CLI envelope，以及 trace-driven `tessera apply-patch --from-trace` CLI envelope；diff/test execution、checkpoint restore/rollback、automatic worktree lifecycle 和 code review command 仍按后续 runtime gate 推进。
...
- Automatic isolated worktree lifecycle for trace-driven apply-patch.
