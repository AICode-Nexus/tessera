...
- v0.7 已具备 metadata-only coding workflow foundation、窄的 explicit `tessera apply-patch` CLI envelope，以及 trace-driven apply-patch automation design；diff/test execution、checkpoint restore/rollback、trace-driven apply automation implementation 和 code review command 仍按后续 runtime gate 推进。
...
- v0.7 metadata foundation 让 GUI 可以只读展示 coding workflow、scope、patch/test/review/restore 证据；当前 apply-patch mutation 只允许通过 explicit CLI envelope，trace-driven apply-patch 仍只是设计 gate，Git/diff/review/patch controls 仍必须等待对应 typed client intent、policy、checkpoint 和 trace event。
...
v0.7 should be the first version that can honestly claim coding-agent workflow. The current foundation can represent and replay workflow scope, patch proposals, patch application records, test plans/runs, review bundles and restore plans through protocol/core/client/GUI bindings. It also has a narrow explicit `tessera apply-patch` CLI envelope for one gated isolated-root file mutation and a trace-driven apply-patch automation design that resolves reviewed mutation bundles from trace, but it still is not a full coding-agent diff/test/checkpoint/Git workflow.

Remaining runtime-complete work:

- Trace-driven apply-patch/edit workflows with diff preview.
