...
- [x] `tessera chat --provider mock` 交互式 CLI REPL：无 `--prompt` 时进入 Claude/Codex 风格命令行聊天壳，启动时显示 active profile / data dir / configured profiles，支持 `/help`、`/new`、`/profiles`、`/profile <id>`、`/sessions`、`/resume <trace_id>`、`/doctor`、`/status`、`/export`、`/quit`，并复用 `tessera-client` projection 与 core live event stream。
