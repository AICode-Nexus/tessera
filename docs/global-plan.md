...
- [x] Safe `ClientWorkflowInspection` projection for read-only diff/review/approval inspection, exposing generated workflow/task refs, counts, booleans and status labels only.
- [x] Read-only `tessera workflow inspect --trace <trace_id> [--json]`, including empty trace output, over replayed `ClientSnapshot.workflow_inspections`.
- [x] TUI `workflow_inspection_summary` status-line rendering and GUI Review Inspection metadata panel over `snapshot.workflow_inspections` / `status.workflow_inspection_summary`, without loading artifact bodies, rendering patch text, running tests, restoring checkpoints, staging/committing/pushing Git, executing provider/tools, starting an app-server listener, or adding GUI/TUI mutation controls.
...
- [~] GUI/TUI coding workflow visibility: compact status and safe read-only metadata panels exist for workflow/worktree/review inspection; artifact-body loading, patch-text rendering and mutation controls remain planned.
- [ ] Swarm scheduler.
...
44. [x] Implement read-only diff/review/approval workflow inspection with safe generated refs/counts/status labels only across client, CLI, TUI and GUI.
45. [!] Do not start hook or automation runtime until tool/policy/sandbox/checkpoint/task ownership gates exist.
46. [!] Do not start diff apply expansion, test runner, checkpoint restore, branch/stage/commit/push/PR Git mutation, GUI mutation controls, app-server mutation listener, swarm, or learning apply until their stated gates exist.
47. [x] Add a metadata-only test evidence summary contract before executable test runner integration.
