...
Read-only worktree lifecycle consumers are part of the v0.7 projection surface: `RuntimeReader::list_worktree_lifecycles`, `tessera-client` `worktree_lifecycles` / `worktree_summary`, `tessera worktree list --trace <trace_id> [--json]`, TUI status summaries and GUI metadata panels may inspect lifecycle status from trace. `trace_cleanup_candidate` is only trace evidence that a generated retained worktree may be eligible for the separate cleanup command; it is not permission to clean up without explicit `--worktree-path` validation. GUI worktree panels must stay safe aggregate views and omit local paths, workflow objective text, reason text, touched paths, summaries, diagnostics, artifact bodies and mutation controls.

Read-only workflow inspection consumers are also part of the v0.7 projection surface: `tessera-client` `ClientWorkflowInspection` / `workflow_inspection_summary`, `tessera workflow inspect --trace <trace_id> [--json]`, TUI status summaries and the GUI Review Inspection panel may inspect diff/review/approval metadata from trace. The projection must use generated refs such as `workflow:1` and `task:1`, counts, booleans and status labels only. It may count patch proposals, diff artifact refs, review bundles/evidence refs, reviewer-gate accepted/rejected/revision-requested/pending state, approvals, apply-patch preflights and apply-patch executions. It must not expose raw workflow/task/reviewer/approval ids, paths, objective text, summaries, diagnostics, comments, restore reasons, artifact bodies, patch text or stdout/stderr bodies, and it must not run tests, restore checkpoints, apply diffs, stage/commit/push Git, execute provider/tool calls, start an app-server listener or add GUI/TUI mutation controls.
