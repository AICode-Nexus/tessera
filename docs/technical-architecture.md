...
- `SubagentSession`：current v0.6 foundation records and projects parent/child task linkage, caps, scope labels, transcript artifact handles, approval forwarding metadata and inactive-child policy before any scheduler, fan-out or child execution runtime exists. It also defines a core-owned, non-executing transcript artifact lifecycle helper for reserved/published/sealed/abandoned handle metadata. Future persistent runtime must add executable scheduler coordination, task ownership, approval-forwarding policy records, inactive-child policy records and cancellation/reattach metadata before child provider calls.
