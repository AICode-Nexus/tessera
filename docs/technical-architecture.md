...
- `IsolatedWorktreeLifecyclePlanner`：v0.7 automatic isolated worktree lifecycle 的非执行 planner。它只根据调用方提供的 source checkout status、repo key、data dir/base option、collision list、retention policy 和 evidence refs 生成 deterministic worktree id、root label、path-safe leaf、本地 path 计划、redacted base key 以及 `planned` lifecycle event。它不发现 Git 状态、不读写文件系统、不创建或删除 worktree、不写 storage、不运行测试/patch/Git、不恢复 checkpoint、不驱动 UI。
- `IsolatedWorktreeLifecycleRunner`：v0.7 automatic isolated worktree lifecycle 的固定 Git 边界。它通过 injectable runner 请求 `git rev-parse HEAD`、`git status --porcelain --untracked-files=no`、`git worktree add --detach` 和非 force `git worktree remove`，并生成 created/retained/cleanup lifecycle metadata；默认 runner 在 spawn 前拒绝任何非白名单 invocation。它不运行 shell、测试、`git apply`、checkout/switch、stage/commit/push/pull/merge/rebase/clean/reset/stash、checkpoint restore、provider 或 UI。
...
- `protocol`：公共类型、ID、runtime schema、RunEvent、EventFrame、NormalizedError、context reference schema、diagnostics schema、memory proposal schema、skill manifest schema、sandbox profile schema、checkpoint schema、coding workflow and worktree lifecycle metadata schema。
...
- `IsolatedWorktreePlan` metadata：automatic worktree mode 先由 core 生成 deterministic id/root label/leaf/base key 和 planned lifecycle event；固定 Git worktree runner 只允许 detached add/remove lifecycle boundary；test/lint、stage/commit/push、branch/PR 和 checkpoint restore 仍留给后续显式 gate。
