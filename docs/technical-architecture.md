...
- `IsolatedWorktreePlan` metadata：automatic worktree mode 先由 core 生成 deterministic id/root label/leaf/base key 和 planned lifecycle event；真实 Git discovery、worktree create/remove、cleanup、test/lint 和 stage/commit/push 仍留给后续显式 gate。
