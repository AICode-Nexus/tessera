...
- `SubagentSession`：current v0.6 foundation records and projects parent/child task linkage, caps, scope labels, transcript artifact handles, approval forwarding metadata and inactive-child policy before any scheduler, fan-out or child execution runtime exists. It also defines core-owned, non-executing helpers for transcript artifact lifecycle metadata, approval-forwarding metadata and inactive-policy metadata. Future persistent runtime must add executable scheduler coordination, task ownership, automatic approval forwarding, inactive-child execution and cancellation/reattach metadata before child provider calls.
