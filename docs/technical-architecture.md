...
- v0.1 不选择最终 GUI toolkit，不引入桌面依赖。
...
- GUI 早期 spike 只允许接 mock 或 read-only runtime，验证布局、状态投影、分发体积和可访问性。

候选方向：

- Tauri：适合未来需要 Web 技术生态、跨平台桌面壳和更复杂 UI 的场景。
- egui：适合 Rust-first、单二进制、轻量配置面板和调试工作台。
- GPUI：适合更原生的高性能桌面体验，但应等生态和项目需求更明确后再决策。

在没有完成 client model 和 live event bridge 前，不应开始真实 GUI 功能开发。
...
| GUI | 先保留 Tauri / egui / GPUI spike，不在 v0.1 固化 | 避免在 runtime 未稳定前引入桌面依赖，先固化 UI-neutral client model |
