...
它不是单纯的聊天 TUI，也不是某个 provider 的客户端。它的核心是一个可审计、可回放、可扩展的本地 runtime。CLI、TUI、replay runner、未来 runtime API、工具系统、agent 系统、memory 和 skill 都必须共享同一套协议和状态模型。

核心目标：

- Model-agnostic：支持多个 provider，但 core 不被 provider 私有结构污染。
- Headless-first：先有可测试的 headless runtime，再有 TUI。
...
User input
  ...
  -> CLI/TUI renders streamed events
...
2. v0.2：context workbench、read-only runtime API、task registry v1、cost/cache telemetry、model router 草案。
...
| Storage | JSONL + SQLite | 可回放事件真相 + 可查询索引 |
