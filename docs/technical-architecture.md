...
- Agent-ready：v0.1 先预留 agent 接入点；当前 v0.5 已加入 no-tool single-agent loop、opt-in project instruction discovery/source reporting、explicit read-only Skill Runtime v1，并完成 background task ownership 的 trace-backed protocol / recorder / projection foundation；工具、executable skills、默认/全局指令加载、background reattach 和多 agent 仍必须按门禁推进。
...
5. v0.5：no-tool single agent loop、non-interactive agent run envelope、opt-in project instruction discovery、skill runtime v1、trace-backed background ownership foundation、app-server alignment、pause/resume、context handle projection。
