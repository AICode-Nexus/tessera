...
- `ApplyPatchGate`：v0.7 preflight gate 只验证 mutation request、patch proposal、scope、checkpoint、reviewer、policy、sandbox 和 worktree metadata 是否满足未来 executor 条件，并持续返回 `executor_blocked`；它不读取或写入 workspace、不创建 worktree、不运行 patch/test/Git 命令、不恢复 checkpoint、不驱动 UI。
