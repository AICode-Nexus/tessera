...
`tessera agent run` starts the first bounded single-agent path. It is script-friendly, writes `TaskKind::AgentRun` plus agent run/step trace events, and returns text or JSON containing trace/task/status/summary fields. Add `--instructions --workspace <path>` to opt into core-owned project instruction discovery; the CLI reports instruction sources and writes `instructions_discovered` metadata without trace content. It does not execute tools, mutate files, activate skills, run hooks, or reattach background work.
