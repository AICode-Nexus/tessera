...
Interactive `chat` mode prints startup context for the active profile, data dir, and configured profiles. It supports `/help`, `/new`, `/profiles`, `/profile <id>`, `/sessions`, `/resume <trace_id>`, `/doctor`, `/status`, `/export`, and `/quit` while keeping provider execution behind core and client projection in `tessera-client`. `chat --list-commands` prints the same command list without resolving config or starting the REPL.
