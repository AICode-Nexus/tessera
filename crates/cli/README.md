...
Interactive `chat` mode prints startup context for the active profile, data dir, and configured profiles. It supports `/help`, `/commands`, `/new`, `/clear`, `/cancel`, `/paste`, `/profiles`, `/profile <id>`, numbered `/sessions`, `/resume <trace_id|#>`, `/doctor`, `/history`, `/status`, `/export`, and `/quit` while keeping provider execution behind core and client projection in `tessera-client`. In paste mode, `/send` submits the collected multiline prompt and `/cancel` discards it; outside paste mode, `/cancel` reports that there is no active run to cancel until the line-oriented REPL gets an async input layer. The underlying CLI helpers already accept core `RunControls` / `RunCancellationToken`, and the TUI bridge uses that path for active stream cancellation. `chat --list-commands` prints the same command list without resolving config or starting the REPL.
