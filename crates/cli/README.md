...
v0.1 exposes `tessera doctor --json`, `tessera init`, one-shot `tessera chat --provider mock --prompt ...`, script-friendly `chat --json`, trace inspection with `transcript` / `replay` / `events`, and interactive `tessera chat --provider mock` / `chat --continue` on top of `tessera-core`.
