...
It also exposes the first read-only runtime API surface: `RuntimeReader` can page trace events, query indexed runtime object IDs, and rebuild task summaries through the core boundary. This is a local Rust API, not the future HTTP/SSE server.
