...
- Added an explicit `tessera apply-patch` CLI command that runs the core apply-patch gate and isolated single-file executor with trace-backed preflight/execution metadata, while keeping worktree creation, tests, checkpoint restore, Git mutation, trace-driven workflow automation, and GUI/TUI mutation controls blocked.
